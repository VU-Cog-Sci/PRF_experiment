...
class SPMapperSession(EyelinkSession):
    def __init__(self, subject_initials, index_number,scanner, tracker_on):
        super(SPMapperSession, self).__init__( subject_initials, index_number)
    ...
    def prepare_trials(self):
        ...
        sp_amplitudes = np.hstack([0,np.tile([1,0],self.standard_parameters['mapper_n_trials']/2),0]) * self.standard_parameters['sp_path_amplitude']
        trial_durations = np.hstack([self.standard_parameters['warming_up_n_TRs'],np.tile([1,1],self.standard_parameters['mapper_n_trials']/2)* self.standard_parameters['mapper_period_in_TR'],self.standard_parameters['warming_up_n_TRs']]) 
        ...
        self.phase_durations = np.array([[
            ...
            ] for i in range(len(sp_amplitudes))] )    
        ...
        self.all_trials = []
        for i in range(len(sp_amplitudes)):#self.trial_order:

            this_trial_parameters={
                                    # trial varying params
                                    'sp_path_amplitude':sp_amplitudes[i],
            ...
            self.all_trials.append(SPMapperTrial(this_trial_parameters, phase_durations = self.phase_durations[i], session = self, screen = self.screen, tracker = self.tracker))

    
    def close(self):
        super(SPMapperSession, self).close()        
