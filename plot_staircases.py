...
def plot_staircases(initials,run_nr):
	...
	for i in range(run_nr):
		...
		color_staircase_1.append([np.concatenate([np.array([float(e.split(' ')[-4]) for e in picklefile['eventArray'][i]  if ('signal in feature: Color ecc bin: ' + str(eccen_bin) in e) * (picklefile['parameterArray'][i]['task_index'] == 0)]) for i in range(len(picklefile['eventArray']))])  )
